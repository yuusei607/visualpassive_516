...
# ------------------ スムージング初期化 ------------------
gaze_history = deque(maxlen=5)  # 5個の視線履歴
R = 0.75  #Rはディスプレイ上のディスプレイと実際のディスプレイの比(R>1)
...
while True:
    ...
    if norm_pos:
        ...
        # --- 視線位置の移動平均（スムージング） ---
#これを消して       #gaze_avg = np.mean(gaze_history, axis=0)
        # 移動平均ではなく指数移動平均にする
#ここから
        alpha = 0.2  # 小さいほど滑らかに
        if len(gaze_history) == 0:
            ...
            gaze_avg = (1 - alpha) * gaze_avg + alpha * gaze
#ここまで編集した
        ...
        pts1_in_mm = (3.8/R) * k2 * np.float32([#4/25に編集1/R→2/R　これにより画像の表示サイズを2倍に
        ...
        if np.linalg.norm(gaze_centered) < 900:
            ...
            x_parallel = 100 #mm
            y_parallel = 0 #mm
            v2 = np.array([[x_parallel,x_parallel,x_parallel,x_parallel],
                [y_parallel,y_parallel,y_parallel,y_parallel]])
