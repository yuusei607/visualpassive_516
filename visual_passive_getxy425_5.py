...
while True:
    ...
    if smoothed_gaze is None:
        smoothed_gaze = gaze_avg
    else:
        smoothed_gaze = alpha * smoothed_gaze + (1 - alpha) * gaze_avg

    gaze_screen = (screen_width * smoothed_gaze[0], screen_height * smoothed_gaze[1])
    ...
    screen_height_in_mm = 398
    screen_distance_in_mm = 1000
    ...
    if np.linalg.norm(gaze_centered) < 900:
        if last_gaze_centered is None or np.linalg.norm(np.array(gaze_centered) - np.array(last_gaze_centered)) > 6:
