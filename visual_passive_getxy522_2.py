...
def new_point_with_retinal_image_invariance(u, v, X, Y, d):
  Y = -Y
  v = -v
  L = math.sqrt(u**2 + v**2 + d**2)
  r = math.sqrt((d*u)**2 + (d*v)**2)
  cos = d/L
  sin = math.sqrt(u**2 + v**2)/L
  cos_ = 1-cos
  n1 = -d*v/r
  n2 = d*u/r


  Rotation_matrix = np.array([[n1*n1*cos_ + cos,n1*n2*cos_,n2*sin],[n1*n2*cos_,(n2**2)*cos_+cos,-n1*sin],[-n2*sin,n1*sin,cos]])
  trans_with_same_length = Rotation_matrix@np.array([[X],[Y],[d]])
  k = d/trans_with_same_length[2]

  new_u = k*trans_with_same_length[0]
  new_v = k*trans_with_same_length[1]
  return new_u, -new_v
