...
def new_point_with_retinal_image_invariance(u, v, X, Y, d):
    point = np.array([u, v, 0.0])

    # 視線方向ベクトル
    gaze_vec = np.array([X, Y, d], dtype=np.float64)
    gaze_norm = np.linalg.norm(gaze_vec)
    if gaze_norm < 1e-5:
        return u, v  # 回転しない

    gaze_dir = gaze_vec / gaze_norm

    # 回転軸（z軸と視線方向ベクトルの外積）
    z_axis = np.array([0.0, 0.0, 1.0])
    axis = np.cross(z_axis, gaze_dir)
    axis_norm = np.linalg.norm(axis)

    if axis_norm < 1e-5:
        return u, v  # 回転軸が無意味なときは回転不要

    axis = axis / axis_norm
    theta = math.acos(np.clip(np.dot(z_axis, gaze_dir), -1.0, 1.0))
